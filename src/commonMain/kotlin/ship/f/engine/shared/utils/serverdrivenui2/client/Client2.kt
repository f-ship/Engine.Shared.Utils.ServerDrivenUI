...
abstract class Client2(open val projectName: String? = null) {
    ...
    val nonReactiveStateMap: MutableMap<StateId2, State2> = mutableMapOf() // TODO this should probably never be used
    ...
    fun update(state: State2, forceUpdate: Boolean = false, renderChain: List<StateId2> = listOf()) { //TODO probably need to find a better way of dynamic tree building incase a view isn't found as a fallback
...
//        if (!currentState.id.isAutoGenerated) tempStateList.add(currentState.id).also { println(tempStateList) }
        ...
        if (state.id.ignoreReactive) nonReactiveStateMap[state.id] = state
    }
